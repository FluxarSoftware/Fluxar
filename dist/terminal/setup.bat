@echo off
    ...
    for %%I in ("%~dp0..\..") do set "CortexScriptPath=%%~fI"
    setx PATH "%CortexScriptPath%;%PATH%" /m

    if %errorlevel% neq 0 (
        echo CortexScript was not successfully installed.
    ) else (
        echo CortexScript was successfully installed and added to the System Path.
    )
    endlocal
